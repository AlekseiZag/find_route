...
class AllTestsCase(TestCase):
    ...
    def test_cbv_delete_views(self):
        response = self.client.delete(reverse('cities:delete',
                                           kwargs={'pk': self.city_A.id}))
        # self.assertTemplateUsed(response, template_name='cities/delete.html')
        self.assertEqual(response.resolver_match.func.__name__,
                         cities_view.CityDeleteView.as_view().__name__)

        self.assertContains(response, 'Город успешно удален.', 1, 302)
    ...
    def test_valid_route_form(self):
        data = {'from_city': self.city_A.id, 'to_city': self.city_B.id, 'cities': [self.city_E.id, self.city_D.id],
                'travelling_time': 9}
    ...
    def test_message_error_more_time(self):
        data = {'from_city': self.city_A.id, 'to_city': self.city_E.id, 'cities': [self.city_C.id],
                'travelling_time': 9}
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, 'Время в пути больше заданного', 1, 200)

    def test_nodata_to_find_route(self):
        data = {}
        response = self.client.get('/find_routes/', data)
        self.assertContains(response, 'Нет данных для поиска', 1, 200)

    def test_message_error_from_cities(self):
        data = {'from_city': self.city_B.id, 'to_city': self.city_E.id, 'cities': [self.city_C.id],
                'travelling_time': 349}
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, 'Муршрут через эти города невозможен', 1, 200)

    def test_message_error_nonexistent_route(self):
        data = {'from_city': self.city_B.id, 'to_city': self.city_B.id,
                'travelling_time': 349}
        response = self.client.post('/find_routes/', data)
        self.assertContains(response, 'Муршрута удовлетворяющего условиям не существует', 1, 200)

    def test_get_absolute_url(self):
        author = City.objects.get(id=1)
        self.assertEquals(author.get_absolute_url(), '/cities/detail/1/')

    def test_get_routes_context(self):
        ...
